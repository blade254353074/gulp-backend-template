(function() {
    ...
    function configMake(page, src, dist, assets) {
        ...
        for (var idx = 0; idx < page.length; idx++) {
            ...
            taskCfg.template = {
                html: taskSrc + 'template/**/*.html',
                src: taskSrc + 'template/**/*.ftl',
                dest: dist + 'ftl'
            };
            if (pageName === 'index') {
...
/*                gulp.task(taskObj.template, function() {
                    return gulp.src(taskConfig.template.src)
                        .pipe(gulp.dest(taskConfig.template.dest))
                        .pipe(notify({
                            message: 'template 更新成功!'
                        }));
                });*/
                ...
                gulp.task(taskObj.js4rev, [taskObj.preRevPublic, taskObj.preRevPage], function() {
                    gulp.src('src/' + pageName + '/js/page/editor/dialogs/**/*.js')
                        .pipe(plumber({
                            errorHandler: errrHandler
                        }))
                        .pipe(uglify()) // 将js混淆并压缩
                        .pipe(gulp.dest(assets + pageName + '/js/page/editor/dialogs')) // 输出文件到dest目录
                    return gulp.src([taskConfig.js.src.cache, '!src/' + pageName + '/js/page/editor/dialogs/**/*.js'])
                        .pipe(plumber({
                            errorHandler: errrHandler
                        }))
                        .pipe(uglify()) // 将js混淆并压缩
                ...
                gulp.task(taskObj.rev, [taskObj.css4rev, taskObj.js4rev], function() {
                    // 将template目录内的静态html压缩并释放到dest目录
                    gulp.src([taskConfig.rev.src, taskConfig.template.html])
                        .pipe(revCollect({
                            replaceReved: true
                        }))
                        .pipe(minifyhtml())
                        .pipe(gulp.dest(taskConfig.html.dest));
                    return gulp.src([taskConfig.rev.src, taskConfig.template.src]) // 读取 rev-manifest.json 文件
                        // 和需要进行资源名替换的文件
                        .pipe(revCollect({
                            replaceReved: true
                        })) // 执行文件内css,js名的替换
                        .pipe(gulp.dest(taskConfig.template.dest + '/' + pageName)); // 替换后的文件输出的目录
                });
